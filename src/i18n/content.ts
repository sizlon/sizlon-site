...
export const content = {
  ko: {
    ...
    home: {
      ...
      description: '매체 7,000곳에서 하루 20만 건을 수집·색인하던 검색 엔지니어가 한국어 검색 품질을 진단하고, 공공 SI 감리 문서를 만들고, 필요한 데이터를 매달 납품합니다. 고정가, 결과물 명시, 대표가 직접 수행.',
      h1: '한국어 검색·문서·데이터 파이프라인, 직접 굴려본 사람이 합니다.',
      lede: '매체 7,000곳에서 하루 20만 건을 수집·색인하던 검색 엔지니어가 검색 품질을 진단하고, 공공 SI 감리 문서를 만들고, 필요한 데이터를 매달 납품합니다.',
      servicesEyebrow: '서비스',
    ...
    services: {
      search: {
        title: '한국어 검색 품질 진단',
        description: 'ES/OpenSearch·Nori 검색 품질 진단 — 2주, 250만원 고정가. 실패 쿼리 골든셋 100건으로 before/after 를 측정하고, 바로 적용 가능한 analyzer·사전·동의어·랭킹 설정을 드립니다.',
    ...
    legal: {
      ...
      terms: {
        ...
        sections: [
          ...
          { h: '서비스와 제품의 제공 방식', p: ['시즐론의 서비스 — 검색 품질 진단, 감리 대응 요구사항 추적표, 데이터 피드 —는 통화로 범위를 정한 뒤 견적서와 개별 계약(NDA 포함)에 따라 수행합니다. 본 사이트의 가격은 범위 확정 전 기준가이며, 견적서가 우선합니다. 데이터 피드의 수집 원칙은 /services/data#principles 에 게시하며 견적서·계약서가 이를 참조합니다.', '라이선스 소프트웨어 — 예: Crawler Platform —는 판매가 아니라 라이선스로 제공되며, 오프라인으로 검증되는 벤더 발급 라이선스로 납품됩니다. 온라인 서비스 — 미리보아 —는 Sizlon이 자체 주소에서 운영하며, 해당 사이트에서 계정을 만들고 그 사이트에 게시된 약관에 따라 이용하십니다. 어느 경우든 본 웹사이트 내용과 상충하면 계약, 라이선스 또는 해당 서비스 약관이 우선합니다.'] },
